...
# 2. TUTAJ WKLEJ SWÓJ KLUCZ API GEMINI:
os.environ["GEMINI_API_KEY"] = "TUTAJ_WKLEJ_SWOJ_KLUCZ_API"

# 3. TUTAJ WKLEJ DANE API IDOSELL:
os.environ["IDOSELL_DOMAIN"] = "wassyl.pl" # Podaj bez https://
os.environ["IDOSELL_API_KEY"] = "TUTAJ_WKLEJ_KLUCZ_API_IDOSELL"

# 4. Zapisywanie aplikacji do pliku app.py
kod_aplikacji = """import streamlit as st
...
IDOSELL_DOMAIN = os.environ.get("IDOSELL_DOMAIN", "")
IDOSELL_API_KEY = os.environ.get("IDOSELL_API_KEY", "")
...
def save_draft():
    dane = {
        "wybrany_temat": st.session_state.wybrany_temat,
        "aktualny_wpis": st.session_state.aktualny_wpis,
        "gotowy_html": st.session_state.gotowy_html,
        "kolaz_b64": st.session_state.kolaz_b64
    }
    ...
    for proba in range(3):
        try:
            ...
            if "429" in str(e) or "Quota" in str(e):
                if proba < 2:
                    st.warning(f"⏳ Chwilowy limit darmowego API Gemini. Aplikacja czeka 30 sekund i spróbuje ponownie automatycznie... (Próba {proba+1}/3)")
                    time.sleep(30)
                    continue # Spróbuj ponownie
            raise e # Jeśli to inny błąd lub wykorzystaliśmy 3 próby, rzuć błędem
...
def wyczysc_json(tekst):
    tekst = tekst.strip()
    znacznik = chr(96) * 3 
    if tekst.startswith(znacznik + 'json'): tekst = tekst[7:]
    elif tekst.startswith(znacznik): tekst = tekst[3:]
    if tekst.endswith(znacznik): tekst = tekst[:-3]
    return tekst.strip()
...
def pobierz_dane_produktow_idosell(lista_id_str):
    '''Funkcja uderza bezpośrednio do API IdoSell i pobiera zdjęcia/url produktów'''
    produkty = []
    ...
    # Budujemy parametry zapytania (IdoSell przyjmuje to jako tablicę)
...
def wyslij_do_idosell_api(payload):
    # Wykorzystujemy endpoint POST /api/admin/v7/entries/entries wg dokumentacji
...
# --- NIESTANDARDOWE STYLE CSS (DOPRACOWANIE WIZUALNE) ---
st.markdown('''
<style>
    ...
    /* Wyróżnienie sekcji sukcesu */
    div.stSuccess {
        border-radius: 8px;
        border-left: 5px solid #28a745;
    }
    
    /* Wyróżnienie sekcji informacyjnej */
    div.stInfo {
        border-radius: 8px;
...
# ŁADOWANIE SZKICU PRZY STARCIE APLIKACJI
...
st.markdown("<h1 style='text-align: center; margin-bottom: 30px;'>Asystent Content Marketingu</h1>", unsafe_allow_html=True)

# --- BOCZNE MENU (Z USTAWIENIAMI API) ---
with st.sidebar:
    st.header("Wybierz tryb pracy:")
    opcje_menu = ("1. Tematy na podstawie trendów", "2. Tematy na podstawie danych", "3. Stwórz wpis", "4. Publikacja IdoSell")
...
if url_sklepu:
    with st.spinner("Czytam podaną stronę..."):
        ...
        if tekst_strony: dodatkowy_kontekst = f"\\nDane ze strony: {tekst_strony}\\n"
...
if opcja in ["1. Tematy na podstawie trendów", "2. Tematy na podstawie danych"]:
    if opcja == "1. Tematy na podstawie trendów":
        ...
        prompt_baza = f"Bazuj na następującym pomyśle/haśle: {dane_wejsciowe}." if dane_wejsciowe else ""
                ...
                try:
                    ...
                    for t in tematy:
                        with st.container():
                            ...
                            st.button(f"✨ Przejdź do realizacji", key=f"btn_{t['tytul']}", on_click=przejdz_do_realizacji, args=(t['tytul'],))
                            st.markdown("---")
                except Exception as e:
...
# === ZAKŁADKA 3 ===
    ...
    st.info("Wszystko, co tu wygenerujesz, jest automatycznie zapisywane w wersji roboczej. Nie stracisz danych po odświeżeniu strony!")
    ...
    if st.button("🚀 Wygeneruj pierwszą wersję wpisu"):
        if temat_wpisu:
            # Zapisz temat przed generowaniem
            st.session_state.wybrany_temat = temat_wpisu
            save_draft()
            with st.spinner("Tworzę artykuł (optymalna długość ok. 4000-5000 znaków)..."):
                prompt_artykul = f'''
{KONTEKST_SKLEPU} {dodatkowy_kontekst}
...
2. Lead (Zajawka): Dowcipny, "edgy" i przykuwający uwagę Gen Z / Millenialsów. BEZWZGLĘDNY WYMÓG: Maksymalna długość leadu to 300 znaków (zaledwie 2-3 krótkie zdania). Musi być bardzo zwięzły.
3. Struktura: Minimum 3 rozbudowane śródtytuły (H2).
4. E-E-A-T: Pokaż doświadczenie szwalni we Wrocławiu i wiedzę o materiałach.
5. Długość i Styl: Skoncentruj się na konkretach. Celuj precyzyjnie w długość pomiędzy 4000 a 5000 znaków ze spacjami. Absolutnie nie przekraczaj 6000 znaków. Pisz zwięźle i na temat.
Format: Markdown.
'''
                try:
                    odpowiedz = generuj_tekst_ai(prompt_artykul)
                    st.session_state.aktualny_wpis = odpowiedz.text
                    save_draft() # Auto-zapis po wygenerowaniu!
                except Exception as e:
                    st.error(f"Błąd: {e}")
        else:
            ...
            if st.button("🔄 Nanieś poprawki"):
                if uwagi:
                    with st.spinner("Aplikuję Twoje poprawki..."):
                        prompt_poprawa = f'''
{KONTEKST_SKLEPU}
Zadanie: Przebuduj ten tekst zgodnie z uwagami od użytkownika: "{uwagi}". 
BEZWZGLĘDNE WYMOGI: Lead (zajawka) nie może przekraczać 300 znaków (2-3 zdania). Cały tekst musi mieć między 4000 a 5000 znaków (absolutne maksimum to 6000). Nie rozpisuj się niepotrzebnie.
Oto aktualny tekst:
{st.session_state.aktualny_wpis}
'''
                        try:
                            ...
                            save_draft() # Auto-zapis po poprawce
                            st.rerun() 
                        except Exception as e:
...
# === ZAKŁADKA 4 ===
    ...
    if not st.session_state.aktualny_wpis:
        ...
        with col_form1:
            ...
            form_produkty_polecane = st.text_input("ID produktów polecanych (oddzielone przecinkami):", placeholder="np. 16216, 16254")
            form_produkty_html = st.text_input("ID produktów do grafik w treści HTML:", placeholder="np. 16216, 16357")
        ...
        with col_form2:
            ...
            with col_wymiar1: kolaz_szerokosc = st.number_input("Szerokość (px)", value=1200, step=10)
            with col_wymiar2: kolaz_wysokosc = st.number_input("Wysokość (px)", value=630, step=10)
                
            if st.button("🖼️ Generuj Kolaż"):
                if form_produkty_kolaz:
                    with st.spinner("Pobieram zdjęcia przez API IdoSell i kleję kolaż..."):
                        ...
                        if produkty_do_kolazu:
                            ...
                            b64_kolaz = generuj_kolaz_b64(lista_url, kolaz_szerokosc, kolaz_wysokosc)
                            if b64_kolaz:
                                st.session_state.kolaz_b64 = b64_kolaz
                                save_draft() # Auto-zapis obrazka
                                st.success("Kolaż gotowy i zapisany w szkicu!")
                        else:
                            st.warning("Nie udało się pobrać żadnych zdjęć ze sklepu. Sprawdź poprawność ID.")
                else:
...
ZASADY: Zwróć TYLKO czysty kod HTML. Każdy akapit w <p style="...">. Każdy nagłówek w <h3 style="...">.
Artykuł:
{st.session_state.aktualny_wpis}
'''
                try:
                    ...
                    if czysty_html.startswith("html"): czysty_html = czysty_html[4:].strip()
                    
                    st.session_state.gotowy_html = czysty_html
                    save_draft() # Auto-zapis HTML!
        ...
        if st.session_state.gotowy_html:
            ...
            if st.button("🚀 WYŚLIJ WPIS DO IDOSELL", type="primary"):
                if not IDOSELL_DOMAIN or not IDOSELL_API_KEY:
                    st.error("Podaj zmienne środowiskowe IDOSELL_DOMAIN i IDOSELL_API_KEY przed wysyłką!")
                else:
                    with st.spinner("Trwa wysyłka do API IdoSell..."):
                        # Tworzenie Payloadu wg dokumentacji IdoSell
                        payload = {
                            "params": {
                                "shopId": 1, # Domyślny ID sklepu, można w przyszłości edytować
                                "visible": "n", # Tworzymy szkic dla bezpieczeństwa (nie publikujemy od razu)
                                "langs": [
                                    {
                                        "langId": "pol",
                                        "title": form_tytul,
                        ...
                        # Dodawanie okładki, jeśli istnieje
                        if st.session_state.kolaz_b64:
                        ...
                        if form_produkty_polecane:
                            id_tab = [int(x.strip()) for x in form_produkty_polecane.split(",") if x.strip().isdigit()]
                            if id_tab:
                        ...
                        if api_kod == 200:
                            st.success(f"🎉 Wpis został pomyślnie utworzony w panelu IdoSell! ID Wpisu: {api_odpowiedz.get('result', {}).get('entryId', 'Brak ID')}")
                            st.balloons()
                            clear_draft() # Czyścimy szkic po udanej wysyłce
                        else:
                            st.error(f"❌ Błąd wysyłki (Kod {api_kod}): {api_odpowiedz}")
"""

with open("app.py", "w", encoding="utf-8") as plik:
    plik.write(kod_aplikacji)

adres_ip = "https://" + "ipv4.icanhazip.com"
ip = urllib.request.urlopen(adres_ip).read().decode('utf8').strip()
print(f"✅ Kod zapisany! TWOJE HASŁO (IP) DO TUNELU TO: {ip}")